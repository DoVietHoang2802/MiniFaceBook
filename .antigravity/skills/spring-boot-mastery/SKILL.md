...
- **Error Handling**: Sử dụng `@RestControllerAdvice` để bắt lỗi toàn cục và trả về format JSON đồng nhất (`ApiResponse<T>`).
- **File Upload Limits & Security**:
  - Giới hạn file tối đa là **5MB** và request tối đa **25MB** thông qua `spring.servlet.multipart` cấu hình trong `application.yml` để tránh lỗi Tomcat `ERR_CONNECTION_RESET`.
  - Luôn sử dụng **Apache Tika (Magic Bytes)** ở Shared Media Infrastructure để quét mã nhị phân của tệp tin tải lên, chặn đứng tệp độc hại ngụy trang.
...
- **Architecture Test**: ArchUnit để đảm bảo Clean Architecture không bị phá vỡ. Luôn sử dụng `.withOptionalLayers(true)` trong ArchUnit test để tránh lỗi build giả trên các package trống.
...
3. **Context Handoff:** Nếu là task cuối của phiên, phải cập nhật `docs/session/SESSION_HANDOFF.md`.
4. **Consistency Check:** Đảm bảo `docs/architecture/SYSTEM_DESIGN.md` phản ánh đúng các thay đổi hạ tầng (Port, DB, Security).
