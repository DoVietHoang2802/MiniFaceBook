...
| **BACKEND_ARCHITECTURE.md** | Đặc tả kiến trúc phân lớp Backend Clean Architecture, Lombok, MapStruct và ArchUnit. |
| **FRONTEND_ARCHITECTURE.md** | Đặc tả kiến trúc phân lớp Frontend, Axios Mutex Interceptors và TypeScript typings. |
| **DATABASE_SCHEMA.md** | Đặc tả chi tiết lược đồ cơ sở dữ liệu MongoDB, Redis và Mongock Migration. |
| **UI_UX_DESIGN.md** | Cẩm nang thiết kế UI/UX Premium: Notion-inspired Light Slate Mode HSL, Google Fonts và Micro-interactions. |
| **CV_PORTFOLIO_HIGHLIGHTS.md** | Bản đồ thành tựu nổi bật (Portfolio Highlights) của cả FE và BE chuẩn STAR. |
...
- **db-optimization:** Tiêu chuẩn tối ưu MongoDB, Redis Caching và Mongock Migration.
