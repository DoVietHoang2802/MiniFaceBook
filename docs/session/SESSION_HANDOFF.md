...
## 🏁 Trạng thái hiện tại: Đã hoàn thành 100% SPRINT 3.1 (Friend Request System) + Việt hóa toàn bộ i18n. Sẵn sàng bước vào SPRINT 3.2: FRIEND LIST & MANAGEMENT.
...
### 🚀 Nhiệm vụ tiếp theo (Sprint 3.2 - Friend List & Management)
...
- File test riêng: `docs/testing/PHASE_3_FRIENDS_TESTING.md`.

**Tiếp theo - Sprint 3.2 (Friend List & Management):**
1. **Backend:**
...
2. **Lưu ý kỹ thuật:** Tái sử dụng `findAcceptedByUserId` và `findByAddresseeIdAndStatus` / `findByRequesterIdAndStatus` đã định nghĩa sẵn trong `FriendshipRepository`. Cần bổ sung thông tin user (avatar, email, bio) vào response — join với `UserRepository`.
