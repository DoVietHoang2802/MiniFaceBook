...
## 🏁 Trạng thái hiện tại: ✅ PHASE 3 HOÀN THÀNH TRỌN VẸN 100% (Sprint 3.1→3.4 + UI). Bao gồm Friend Request, List/Block, Search, Friend Suggestions (Mutual Friends). Sẵn sàng bước vào PHASE 4: REALTIME CHAT.
...
- Sprint 4.1: WebSocket Foundation (Spring WebSocket + STOMP, SockJS fallback, Redis presence Online/Offline, JWT handshake auth).
- Sprint 4.2: Chat Infrastructure (Conversation + Message entity, denormalize lastMessage).
- Sprint 4.3: Messaging Logic (Sent/Delivered/Seen, Optimistic UI).
- Sprint 4.4: Chat UX (Typing indicator, reactions, reply, media).
