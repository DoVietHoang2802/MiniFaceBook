...
|---------|------|---------|
| 4.7 | Jul 2026 | **Profile UI/UX Refinements, Friend Action Dropdown & Social Data Reset Prevention — Sprint 8.6 & UX Fixes HOÀN THÀNH 🏆**: Khắc phục triệt để lỗi xóa trắng danh sách bài viết & bạn bè khi cập nhật thông tin cá nhân bằng `loadedProfileIdRef` guard. Ẩn dòng vai trò hệ thống (`QUYỀN HẠN HỆ THỐNG`), chuẩn hóa nút "Chỉnh sửa trang cá nhân", nâng cấp nút "Bạn bè (Hủy kết bạn)" thành Dropdown Menu `✓ Bạn bè 🔽` chuẩn 100% Facebook UI/UX. Build thành công 100%. |
| 4.6 | Jul 2026 | **Admin Portal & E2E WebRTC Call Coverage — HOÀN THÀNH Sprint 8.6 🎉**: Xây dựng trọn bộ Trang Quản trị Admin Dashboard (`/admin`) gồm 4 phân hệ (KPI Analytics, User Ban/Unban/Role, Content Moderation xóa bài vi phạm, Phát thông báo toàn hệ thống System Broadcast). Bổ sung E2E Test case cho WebRTC Voice Call trong `chat.spec.ts`. Đồng bộ 100% tài liệu kiến trúc. |
| 4.5 | Jul 2026 | **UI Refinements, ProfilePage Real Data & Full E2E Test Coverage — Sprint 6.5 HOÀN THÀNH 🏆**: Xóa nút search tròn và nút Tùy chọn hội thoại trong ChatPage header. Cập nhật sidebar trang cá nhân hiển thị dữ liệu thật (tên, công việc, thành phố, ảnh, bạn bè). Viết mới `profile.spec.ts` (6 tests) và cập nhật `chat.spec.ts` (6 tests Playwright E2E). Thực thi "Update Full" đồng bộ 7 file tài liệu kiến trúc. |
